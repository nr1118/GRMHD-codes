...
{
public:
	void Center_diff(const double dx, std::vector<Data_Mesh<double>>& U, std::vector<Data_Mesh<double>>& dUdx)
	{
		for (int i = 0; i < U.size(); i++)
		{
			dUdx[i] = (U[i + 1] + U[i - 1]*-1.0)* (1/dx);
		}
	}
	void Rhs( const double dx,const double c,const std::vector<Data_Mesh<double>>& U, std::vector<Data_Mesh<double>>& dUdt) const
	{
		for (int i = 0; i < U.size(); i++)
		{
			dUdt[i] = U[i] * c; // this says that dUdt = -U(t) which we know the solution of
		}
	}
...
public:
	...
	void Take_time_step_Euler(const double dx,const double c,const double dt, std::vector<Data_Mesh<double>>& U, const RhsComputation& RhsComp)
	{
		std::vector<Data_Mesh<double>> dUdt(U);

		RhsComp.Rhs(dx,c,U,dUdt);
		for (int i = 0; i < U.size(); i++)
		{
			U[i] = U[i]+ dUdt[i] * dt;
		}
