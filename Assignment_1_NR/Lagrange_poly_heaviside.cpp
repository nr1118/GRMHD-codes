...
// first space is to create an empy vector for the data, dx is delta x, x is the x in which you which to solve, n is the number of terms for the expansion 
{
	for (int k = 0; k < n; k++)
	{
		double f_xi = dx * k - double(0.15);

		if (f_xi > 0)
		{
			f_interp.push_back(float(1.0));
		}
		if (f_xi <= 0)
		{
			f_interp.push_back(float(0.0));
		}
		//this is our f(x_i) that we know at certain points
	}
...
int main()
{
	...
	for (double x : x_given)
	{
		...
		double f_x = x - double(0.15); //f(x)
		double f_tilde = Interp(f_interp, dx, x, n); //f_tilde(x)

		if (f_x > 0)
		{
			double diff = f_tilde - double(1.0); // Our function
			if (x > dx)
			{
				std::cout << x << ",";
				std::cout << diff << "\n";
		...
		if (f_x <= 0)
		{
			double diff = f_tilde - double(0.0); // Our function
			if (x > dx)
			{
				std::cout << x << ",";
				std::cout << diff << "\n";
