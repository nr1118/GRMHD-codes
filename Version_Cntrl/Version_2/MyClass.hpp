...
public:
	...
	void Rhs_Center_diff( const double dx,const double c,const std::vector<Data_Mesh<double>>& U, std::vector<Data_Mesh<double>>& dUdt) const
	{
		for (int i = 0; i < U.size(); i++)
		{
			dUdt[i] = (U[i + 1] + U[i-1]) * (c /(2* dx)); // this says that dUdt = -
		}
	}
...
public:
	...
	{
		RhsU::Rhs_Center_diff(dx,c,U, dUdt);
	}
