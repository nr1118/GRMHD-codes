...
public:
	...
	void Take_time_step_RK2(const int N,double & t_0, const double& dx, const double& c, const double& dt, std::vector<Data_Mesh<double>>& U, const RhsComputation& RhsComp, const Scheme& Method)
	{
		...
		for (int i = 0; i < U.size(); i++)// for each coordinate so in this case it will just be 1
		{
			...
			{
				t_0 = t_0 + (dt/2);
				RhsComp.Rhs(dx, c, U, dUdt[n], Method); // computes dUdt then updates it for each 
														//// element of dUdt this will be our k1,k2
	...
	// The link I am referencing to do the RK2 is this:  https://web.mit.edu/10.001/Web/Course_Notes/Differential_Equations_Notes/node5.html
	//TO DO: why is U[i] not updating?

	//Did you check what U is after each iteration of n or k? yes and I can see no change in U.
	//Did you check on what dUdt looks like after you execute RhsComp.Rhs? Yes after ach iteration of n dUdt does not change which means
	// from the first n to the next when what we want is that dUdt to be an initial then the next time step right?

};


//This is just used for when I want to print stuff
//for (Data_Mesh<double>x : U)
//{
