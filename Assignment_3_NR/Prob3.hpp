...
public:
	...
	void Take_time_step_RK2_loop(const int N, double& t_0, const double& dx, const double& c, const double& dt, std::vector<Data_Mesh<double>>& U, const RhsComputation& RhsComp, const Scheme& Method) const
	{
		...
		for (int i = 0; i < U.size(); i++)// for each coordinate so in this case it will just be 1
		{
				for (int n = 0; n < N; n++)
				{
					RhsComp.Rhs(dx, c, U, dUdt[n], Method);
					for (int k = 0; k <= n; k++)
					{
